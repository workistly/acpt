...
const SENDGRID_EMAIL_USER_REGISTER = defineSecret('SENDGRID_EMAIL_USER_REGISTER')
const STRIPE_API_KEY = defineSecret('STRIPE_API_KEY')
import { Request, Response } from 'express'

exports.updateUser = onRequest(async (request: any, response: any) => {
  cors(request, response, async () => {
    const { uid, email, firstName, lastName, password, closePage, attemptedExams, userId } = request.body
    try {
      if (closePage) {
...
export const resetAttemptedExams = onSchedule({ schedule: '0 0 * * *' }, async () => {
...
export const checkAccountExists = onRequest(async (request: any, response: any) => {
  cors(request, response, async () => {
...
export const setRegistrationsAnalytics = onSchedule({ schedule: '50 23 * * *' }, async () => {
  let date = new Date()
  let day = date.getDate()
  let month = date.getMonth() + 1
  let year = date.getFullYear()
  ...
  for (let i = 0; i < documents.length; i++) {
    ...
    const regDate =
      data && data.createdAt && data.createdAt.seconds ? convertTimestampToDate(data.createdAt.seconds) : ''
...
export const updateRegistrationsAnalytics = onRequest(async (request: any, response: any) => {
  cors(request, response, async () => {
    let date = new Date()
    let day = date.getDate()
    let month = date.getMonth() + 1
    ...
    try {
      ...
      if (dataToStore) {
        // Add the document to the collection
        const docExist = await db.collection('analytics_registrations').where('date', '==', currentDate).get()
        if (docExist.docs.length > 0) {
          const docRef = db.collection('analytics_registrations').doc(docExist.docs[0].id)
...
export const updateExamsAnalytics = onRequest(async (request: any, response: any) => {
  cors(request, response, async () => {
    try {
      let date = new Date()
      let day = date.getDate()
      ...
      for (let i = 0; i < documents.length; i++) {
        const data = documents[i]
        const user: any = userDocuments.find((item: any) => item.docId === data.user_id)

        const regDate = data && data.completed_at && data.completed_at !== '' ? convertExamsDate(data.completed_at) : ''
        ...
        if (currentDate && regDate && currentDate == regDate) {
          ...
          if (data.status && data.status.toString().toLowerCase() === 'complete') {
      ...
      if (dataToStore) {
        const docExist = await db.collection('analytics_exams').where('date', '==', currentDate).get()

        if (docExist.docs.length > 0) {
          const docRef = db.collection('analytics_exams').doc(docExist.docs[0].id)
          await docRef.update(dataToStore)
        } else {
...
export const setExamsAnalytics = onSchedule({ schedule: '50 23 * * *' }, async () => {
  let date = new Date()
  let day = date.getDate()
  let month = date.getMonth() + 1
  let year = date.getFullYear()
  ...
  for (let i = 0; i < documents.length; i++) {
    ...
    if (currentDate && regDate && currentDate == regDate) {
      ...
      if (data.payment && data.status) {
        if (data.payment.toString().toLowerCase() === 'paid' && data.status.toString().toLowerCase() === 'complete') {
          completedAndPaid = completedAndPaid + 1
        } else if (
          data.payment.toString().toLowerCase() === 'paid' &&
          data.status.toString().toLowerCase() !== 'complete'
  ...
  async (request: any, response: any) => {
    cors(request, response, async () => {
      ...
      return await mail
        .send(mailData)
        .then(async () => {
          const emailsDoc = await db.collection('users').where('email', 'in', emails.split(',')).get()
          const batch = db.batch()
  ...
  cors(request, response, async () => {
    ...
    try {
      ...
      querySnapshot.forEach((doc: any) => {
        ...
        if (docData.paid) {
          if (docData.expiryDateTime && new Date(docData.expiryDateTime) < new Date()) {
            expired = true
          } else {
            docsArr.push({
...
exports.getCertificateDataById = onRequest(async (request: any, response: any) => {
  cors(request, response, async () => {
...
exports.addIncompleteExam = onRequest(async (request: any, response: any) => {
  cors(request, response, async () => {
    const { user_id, examId, language, examsAttempted, examsCompleted } = JSON.parse(request.body)
    try {
      const batch = db.batch()
...
exports.verifyCertificate = onRequest({ secrets: [SENDGRID_API_KEY] }, async (request: Request, response: Response) => {
  // Handle CORS
  cors(request, response, async () => {
    ...
    try {
      ...
      logger.info(`Certificate verified successfully for certificateNumber=${certificateNumber}, userName=${userName}`)
      return response.json({ status: true })
    } catch (error: unknown) {
